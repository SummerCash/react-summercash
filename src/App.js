...
class App extends Component {
      ...
      fetch(
        ...
        .then(response => {
          ...
          window.setInterval(() => {
            ...
            this.fetchLastTxHash(this.state.username).then(() => {
              if (this.state.lastTxHash !== oldTxHash) {
                // Check did change
  ...
  // render
  render() {
    if (this.state.username === "not-signed-in" || this.state.username === "") {
      // Check not signed in
    ...
    if (!this.state.hasCookie && userAgent.indexOf(" electron/") === -1) {
      // Has not accepted cookie
      cookieBanner = (
        <CookieBanner
    ...
    return (
      <Grommet theme={theme} full>
        ...
        {cookieBanner}
        <Box
          margin={{ top: "large", left: "large", right: "large" }}
          align="center"
          direction="row"
        >
        ...
        {this.state.showRedeemableModal
          ? this.showRedeemable(512, 535, true)
          : null}
  ...
  fetchBalancePure(username) {
    return fetch(
      "https://summer.cash:3033/api/accounts/" + username + "/balance",
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json"
  ...
  fetchLastTxHash(username) {
    return fetch(
      "https://summer.cash:3033/api/accounts/" + username + "/lastHash",
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json"
  ...
  fetchBalance(username) {
    fetch("https://summer.cash:3033/api/accounts/" + username + "/balance", {
      method: "GET",
      headers: {
        "Content-Type": "application/json"
      }
      ...
      <Media query="(min-width:588px)">
        {matches =>
          matches ? (
            ...
            >
              <Box
                margin={{ right: "medium", top: "small", bottom: "none" }}
                alignContent="end"
                align="end"
                responsive={false}
              >
                <Close
                  onClick={() => this.setState({ showRedeemModal: false })}
                  cursor="pointer"
                />
              </Box>
              <Box
      ...
      <Media query="(min-width:544px)">
        {matches =>
          matches ? (
            ...
            >
              <Box
                margin={{ right: "medium", top: "small", bottom: "small" }}
                alignContent="end"
                align="end"
              >
                <Close
  ...
  makeRedeemableAccount(burn_rate) {
    ...
    var redeemableUsername =
      this.state.username +
      "_" +
      Math.random()
        .toString(36)
        .substring(7); // Generate redeemable username

    if (burn_rate !== undefined && burn_rate && burn_rate !== 0) {
      // Check has burn rate
      redeemableUsername =
        this.state.username +
        "_" +
        Math.random()
          ...
          .substring(7) +
        "_burnrate:" +
        burn_rate; // Add burn rate
    }

    var redeemablePassword = sha3_512(
      Math.random()
        .toString(36)
        .substring(7)
    ...
    return fetch(
      "https://summer.cash:3033/api/accounts/" + redeemableUsername,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
    ...
    return (
      ...
      >
        ...
        <div id="print-contents">
          <Box align="center" alignContent="center" direction="column">
            <Heading size="small" margin={{ top: "none" }}>
              Scan in SummerCash Wallet
            </Heading>
            <Media query={this.getQuery(startingMediaSize)}>
              {matches =>
                matches ? (
                  <QRCode
                    value={
                      this.state.redeemableAccount.username +
                      "_" +
                      this.state.redeemableAccount.password
                    }
                    size={startingSize}
                  />
                ) : (
  ...
  printContents() {
    ...
    domtoimage
      .toJpeg(node, { bgcolor: "white" }) // Generate PNG image
      .then(dataUrl => {
        print({
          printable: dataUrl,
          type: "image",
          imageStyle:
            "width:70%; height:70%;transform:rotate(-90deg);margin-top:-5%;margin-left: 10%;"
        }); // Print
      })
  ...
  handleScan(scan) {
    if (scan && !this.state.hasAlreadyScanned) {
      // Check scanned
      this.setState({ hasAlreadyScanned: true }); // Has already scanned

      if (!scan.includes("@") && !scan.includes("0x") && !scan.includes("_")) {
        // Check is not tx
        this.errorAlert(
          "Invalid QR code (must be @username or 0x1234 address)"
        ); // Alert
        ...
        if (scan.includes("_burnrate")) {
          // Check has burn rate
          redeemableAccount = {
            username:
              scan.split("_burnrate:")[0] +
              "_burnrate:" +
              scan.split("_burnrate:")[1].split("_")[0],
        ...
        fetch(
          ...
          .then(response => {
            if (response.error) {
              ...
              this.fetchBalancePure(redeemableAccount.username).then(
                balance => {
                  redeemableBalance = balance; // Set redeemable balance

                  if (redeemableAccount.username.includes("_burnrate:")) {
                    // Check has burn rate
                    redeemableBalance = parseFloat(
                      redeemableAccount.username.split("_burnrate:")[1]
                    ); // Set redeemable balance
                  }

                  fetch(
                    "https://summer.cash:3033/api/transactions/NewTransaction",
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json"
  ...
  onSubmitTx(event) {
    ...
    if (formData.amount < 0) {
      ...
      this.errorAlert(
        "Invalid transaction amount; cannot send negative amounts of SummerCash."
      ); // Alert

      return; // Return
    }

    if (this.state.shouldMakeRedeemable) {
      // Check is processing elsewhere
      this.setState({
        shouldMakeRedeemable: false,
      ...
      this.makeRedeemableAccount(formData.burn_rate)
        .then(() =>
          fetch("https://summer.cash:3033/api/transactions/NewTransaction", {
            method: "POST",
            headers: {
              "Content-Type": "application/json"
            },
        ...
        .then(async response => {
          if (response.error) {
            ...
            for (i = 0; i < response.transactions.length; i++) {
              // Iterate through txs
              if (
                !this.state.alreadyReceivedHashes.includes(
                  response.transactions[i].hash
                ) &&
                (response.transactions[i].recipient === this.state.username ||
                  response.transactions[i].recipient === this.state.address)
              ) {
                // Check new hash
                ...
                var newAlreadyReceivedHashes = [
                  ...this.state.alreadyReceivedHashes
                ]; // Clone already received

                newAlreadyReceivedHashes.push(response.transactions[i].hash); // Append hash

                this.setState({
                  alreadyReceivedHashes: newAlreadyReceivedHashes
  ...
  renderTransactions() {
    ...
    for (x = 0; x < this.state.transactions.length; x++) {
      ...
      if (
        x > 0 &&
        this.state.transactions[x - 1].hash ===
          this.state.transactions[x].hash &&
        (this.state.transactions[x].recipient === this.state.address ||
          this.state.transactions[x].recipient === this.state.username) &&
        (this.state.transactions[x].sender === this.state.address ||
          this.state.transactions[x].sender === this.state.username)
      ) {
      ...
      if (this.state.transactions[x].payload) {
        // Check has payload
        ...
        case "send":
          transactionViews.push(
            <TransactionView
              ...
              shortTimestamp={
                this.state.transactions[x].time.toString().split(" ")[0]
              }
              recipient={this.state.transactions[x].recipient
                .toString()
                .substring(0, max)}
        ...
        case "receive":
          transactionViews.push(
            <TransactionView
              ...
              sender={this.state.transactions[x].sender
                .toString()
                .substring(0, max)}
              amount={this.state.transactions[x].amount}
              message={payload}
